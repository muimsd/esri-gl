    ...
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column' }}>
      <div style={{ padding: '10px', background: '#f5f5f5', borderBottom: '1px solid #ddd' }}>
        ...
        <div style={{ marginBottom: '10px', padding: '10px', background: '#e3f2fd', borderRadius: '4px' }}>
          <strong>Instructions:</strong> Click anywhere on the map to identify pixel values at that location.
          {clickLocation && (
            <div style={{ marginTop: '5px' }}>
              <strong>Last click:</strong> {clickLocation.lng.toFixed(4)}, {clickLocation.lat.toFixed(4)}
            </div>
          )}
        </div>
        ...
        {identifyResults && (
          <div style={{ marginTop: '10px', padding: '10px', background: 'white', borderRadius: '4px', maxHeight: '250px', overflowY: 'auto' }}>
            {identifyResults.error ? (
              ...
              <>
                <div style={{ marginBottom: '10px' }}>
                  <strong>Pixel Results:</strong> {identifyResults.results?.length || 0} values found
                </div>
                {identifyResults.results?.map((result, index) => (
                  <div key={index} style={{ marginBottom: '10px', padding: '8px', background: '#f8f9fa', borderRadius: '3px' }}>
                    ...
                    {result.location && (
                      <div>
                        <strong>Location:</strong> {result.location.x.toFixed(4)}, {result.location.y.toFixed(4)}
                      </div>
                    )}
