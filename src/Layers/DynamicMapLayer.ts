...
export class DynamicMapLayer extends RasterLayer {
  ...
  protected _buildExportParams(): Record<string, unknown> {
    ...
    if (options.layerDefs) {
      params.layerDefs = typeof options.layerDefs === 'string'
        ? options.layerDefs
   ...
   */
  protected async _requestExport(params: Record<string, unknown>, bounds: [number, number, number, number]): Promise<void> {
