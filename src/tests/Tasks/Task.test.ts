...
class TestableTask extends Task {
  public get testService() { return this._service; }
  public get testOptions() { return this.options; }
  public get testParams() { return this.params; }
  public get testPath() { return this.path; }
  public get testSetters() { return this.setters; }
  
  // Expose _request method for testing
  public testRequest(method: string, path: string, params: Record<string, unknown>, callback: (error?: Error, data?: unknown) => void) {
    (this as unknown as { _request: (method: string, path: string, params: Record<string, unknown>, callback: (error?: Error, data?: unknown) => void) => void })._request(method, path, params, callback);
  }
...
describe('Task', () => {
  const mockedCleanTrailingSlash = cleanTrailingSlash as jest.MockedFunction<typeof cleanTrailingSlash>;
  ...
  describe('Constructor', () => {
    it('should initialize with URL string', () => {
      ...
      expect(cleanTrailingSlash).toHaveBeenCalledWith('https://example.com/arcgis/rest/services/Test/MapServer/');
    });
  ...
  describe('Generated Setter Methods', () => {
    ...
    it('should allow chaining of setter methods', () => {
      ...
      const chainedResult = (result as unknown as Record<string, unknown>).anotherSetter as (value: unknown) => Task;
  ...
  describe('Request Method with Callback', () => {
    ...
    it('should use service.requestWithCallback when service is available', () => {
      ...
      expect(mockService.requestWithCallback).toHaveBeenCalledWith('POST', '/test', task.testParams, callback);
    });
  ...
  describe('Request Method with Promise', () => {
    ...
    it('should use service.requestWithCallback when service is available', async () => {
      ...
      expect(mockService.requestWithCallback).toHaveBeenCalledWith('POST', '/test', task.testParams);
