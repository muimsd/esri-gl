    ...
    it('should handle identify operations across multiple services', async () => {
      mockFetch
        .mockResolvedValueOnce({
          ...
          json: async () => ({
            results: [
              { layerId: 0, layerName: 'States', value: 'California', attributes: { STATE_NAME: 'California' } }
            ]
          })
        } as Response)
        .mockResolvedValueOnce({
      ...
      const results1 = await identifyTask1.at({ lng: -120, lat: 35 });
      const results2 = await identifyTask2.at({ lng: -120, lat: 35 });

      expect(results1).toBeInstanceOf(IdentifyFeatures);
      expect(results2).toBeInstanceOf(IdentifyImage);
