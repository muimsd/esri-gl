...
export class FeatureService {
  ...
  private async _createSource(): Promise<void> {
    try {
      ...
      if (
        this.esriServiceOptions.getAttributionFromService !== false &&
        this._serviceMetadata?.copyrightText
      ) {
  ...
  private _buildQueryUrl(): string {
    ...
    if (options.layers !== undefined) {
      const layerIds = Array.isArray(options.layers)
        ? options.layers.join(',')
  ...
  private _buildQueryUrlWithOptions(options: Partial<FeatureServiceOptions>): string {
    ...
    if (mergedOptions.layers !== undefined) {
      const layerIds = Array.isArray(mergedOptions.layers)
        ? mergedOptions.layers.join(',')
        : mergedOptions.layers.toString();
      params.append('layers', layerIds);
    }
    ...
    if (mergedOptions.maxRecordCount)
      params.append('maxRecordCount', mergedOptions.maxRecordCount.toString());
